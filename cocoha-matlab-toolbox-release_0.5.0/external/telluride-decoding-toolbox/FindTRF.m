...
%                      Uses glmnet (web.stanford.edu/~hastie/glmnet_matlab/index.html)
%                      K(2) controls the degree of L1/L2 penalty ratio (elastic net)
%                      Lasso may result in long computation times
%                 - 'ADMMLasso', K (default K=[.1 1 1], range [0:1, 0:Inf 1:1.8]) : solves lasso with ADMM
%                      ADMM : Aletrnating Direction Method of Multipliers
%                      Uses matlab's lasso (needs statistics toolbox).
%                      K(1) controls the sparsity (number of nonzero coefficients).
%                          Bigger K(1) value -> more zero coefficients
%                      K(2) - the augmented Lagrangian parameter
%                      K(3) - the over-relaxation parameter
%                      In contrast to lasso found in statistics toolbox, 
%                          ADMM-lasso can solve rather large problems quickly
%                          on standard SW and HW, and results in short computation time.
...
if ~isempty(stimulus)
    if strcmp(Method,'lasso') && ~exist('glmnet', 'file')
        error('GLMNET toolbox needed for lasso.');
    end
...
%     XY = Y' * X;  
    ...
    switch Method
        ...
        case 'lasso'
            ...
            g = zeros(size(X,2),ny);
            for ii=1:ny
                for jj = 1:seglen:size(X,1)
                    ix = jj:jj+seglen-1;
                    fit = glmnet(X(ix,:),Y(ix,ii),'gaussian',opt);
                    g(:,ii) = g(:,ii)+fit.beta;
                end
                g = g./seg;
                %fit = glmnet(XX',XY(ii,:)','gaussian',opt);
                %g(:,ii) = fit.beta;
            end
        case 'admmlasso'
            if length(K)==1,K(2)=1;K(3)=1;end
            for ii=1:size(XY,1)
                [~, g(:,ii)] = ADMM_lasso(XX,XY(ii,:),'Lambda',K(1),'Rho', K(2),...
                    'Alpha',K(3),'AbsTol',1e-4, 'RelTol',1e-2);
            end
        case 'none'
            XX = gpuConvert(XX,gpu); XY = gpuConvert(XY,gpu);
            g=gpuGather(XX\XY',gpu);
